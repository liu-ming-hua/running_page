...
const RunMap = ({
  ...
  const hasLoggedMapSuccessRef = useRef(false);
      ...
      const handleStyleLoad = () => {
        ...
        setTimeout(() => {
          try {
            ...
            if (!hasLoggedMapSuccessRef.current) {
              hasLoggedMapSuccessRef.current = true;
              console.info(`✅ Map tiles loaded successfully (${mapStyle})`);
            }
          } catch (error) {
  ...
  return (
    ...
    >
      {mapLoaded && !mapError && (
        <div className={styles.mapSuccessNotification}>Map loaded</div>
      )}
      {mapError && (
