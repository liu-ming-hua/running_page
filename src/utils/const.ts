...
const CHINESE_INFO_MESSAGE = (yearLength: number, year: string = 'Total'): string => {
  const yearStr = year === 'Total' ? '所有' : ` ${year} `;
  return `跑步 ${yearLength} 年，归来仍是6分配`;
};
...
const CHINESE_LOCATION_INFO_MESSAGE_FIRST =
  '走るの好きか。';
const CHINESE_LOCATION_INFO_MESSAGE_SECOND = 'この世界を僕が守る！';
...
const nike = 'rgb(238, 136, 223)'; // if you want to change the main color, modify this value in src/styles/variables.scss
